...
MODEL_DIR         = "model_ddqn_w_huber_loss"
...
def optimize():
    ...
    # 5) 손실 계산 및 역전파
    loss = nn.functional.smooth_l1_loss(q_values, target_q)       # Huber 손실 사용
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
...
plt.xlabel("Episode")
plt.ylabel("Value")
plt.title("CartPole DDQN_w_huber_loss Training (Reward & Q-mean)")
plt.ylim(0, 510)
plt.grid()
plt.legend()
plt.tight_layout()
plt.savefig("cartpole_ddqn_w_huber_loss_rewards_qmean.png")
plt.show()
