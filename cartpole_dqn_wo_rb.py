...
# === 최적화 함수 ================================================
def optimize(state, action, reward, next_state, done):
    """단일 transition으로 바로 Q-Network 학습"""
    ...
    # 현재 Q값
    q_value = policy_net(state_t).gather(1, action_t)

    # 타깃 Q값
    with torch.no_grad():
        next_q_max = target_net(next_state_t).max(1, keepdim=True)[0]
        target_q = reward_t + (1 - done_t) * GAMMA * next_q_max
...
for episode in range(1, TOTAL_EPISODES + 1):
    ...
    while not done:
        ...
        # 단일 transition으로 학습
        q_mean = optimize(state, action, reward, next_state, done)
...
plt.xlabel("Episode")
plt.ylabel("Value")
plt.title("CartPole DQN_wo_rb Training (Reward & Q-mean)")
plt.ylim(0, 510)
plt.grid()
plt.legend()
plt.tight_layout()
plt.savefig("cartpole_dqn_wo_rb_rewards_qmean.png")
plt.show()
